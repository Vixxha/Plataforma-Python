...
# hint: Utiliza una lista de comprensión o filter() para el filtrado y el método .sort() o sorted() con una función lambda para el ordenamiento.
...
try:
    data = [
        ...
        {'nombre': 'Mouse', 'precio': 45, 'stock': 3},
        {'nombre': 'Laptop', 'precio': 800, 'stock': 10}
    ]
    assert procesar_inventario(data) == ['Teclado', 'Mouse', 'Monitor'], "Error: el test 1 ha fallado."
    assert procesar_inventario([]) == [], "Error: el caso de lista vacía falló."
    assert procesar_inventario([{'nombre': 'A', 'precio': 10, 'stock': 10}]) == [], "Error: no debe incluir productos con stock >= 5."
except NameError:
