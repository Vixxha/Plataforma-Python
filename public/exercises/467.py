# === METADATA ===
# title: Analizador de Palíndromos Limpio
# description: Crea una función que determine si una cadena de texto es un palíndromo, ignorando espacios, signos de puntuación y diferencias entre mayúsculas y minúsculas.
# difficulty: Intermedio
# expected_output: True (para "Anita lava la tina")
# hint: Utiliza un bucle o una lista por comprensión para filtrar solo caracteres alfanuméricos antes de comparar la cadena con su versión invertida mediante slicing [::-1].

# === SOLUTION ===
def es_palindromo(texto):
    caracteres_limpios = "".join(caracter.lower() for caracter in texto if caracter.isalnum())
    return caracteres_limpios == caracteres_limpios[::-1]

# === TESTS ===
try:
    assert es_palindromo("Anita lava la tina") == True, "Error: el test 1 ha fallado."
    assert es_palindromo("Hola, ¿cómo estás?") == False, "Error: considera casos límites en tu lógica."
    assert es_palindromo("A man, a plan, a canal: Panama") == True, "Error: el caso base falló."
except NameError:
    raise AssertionError("La función solicitada no está definida. Verifica el nombre.")